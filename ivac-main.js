/* ivac-main.js — ALLEX IVAC Automation (clean, no-bypass)
 * - Floating panel (draggable), ALLEX branding
 * - mobile-verify: **native site form submit** (keeps hidden/CSRF + turnstile)
...
(() => {
    ...
    function startTurnstileWatcher() {
        function snapshot() {
            const inp = document.querySelector('input[name="captcha_token"]') || document.querySelector('input[name="cf-turnstile-response"]');
    ...
    function focusTurnstileWidget() {
        ...
        if (w) { w.scrollIntoView({ behavior: "smooth", block: "center" }); w.style.outline = "3px solid #f59e0b"; setTimeout(() => w.style.outline = "", 1500); } else toast("Turnstile widget not visible here.", false);
    }
    ...
    function nativeMobileVerify(number) {
        ...
        if (!form) {
            const authBox = document.querySelector('.tab-pane.active') || document.querySelector('#authentication, .tab-content') || document.body;
            form = authBox && authBox.querySelector('form');
        }
        if (!form) { toast("Auth form not found. Update selector.", false); return false; }

        const mobileInp = form.querySelector('input[name="mobile_no" i]') || document.querySelector('input[name="mobile_no" i]');
        if (mobileInp) {
        ...
        const t = localStorage.getItem(TS_KEY) || (document.querySelector('input[name="captcha_token"]') || {}).value || (document.querySelector('input[name="cf-turnstile-response"]') || {}).value || "";
        let tsInp = form.querySelector('input[name="captcha_token" i]') || form.querySelector('input[name="cf-turnstile-response" i]');
    ...
    async function postFormMulti(kind, data, needTS = false, includeAuth = false) {
        ...
        for (const base of ROUTE_BASES) {
            ...
            try {
                ...
                if ([403, 419, 429, 503].includes(res.status)) { lastErr = { ok: false, msg: `HTTP ${res.status}` }; return lastErr; }
            ...
            } catch (e) { active.delete(id); lastErr = { ok: false, msg: e.message || "Network" }; continue; }
        }
    ...
    function injectUI() {
        if (document.querySelector(".alx-root")) return;
    ...
    /************** VIEWS **************/
    function row(...children) { const d = document.createElement("div"); d.className = "alx-row";
        children.forEach(c => d.appendChild(c)); return d; }

    function button(txt, cls, fn) { const b = document.createElement("button"); b.className = `alx-btn ${cls}`; b.textContent = txt;
    ...
    function loginView() {
        ...
        const row1 = row(button("SEND VERIFICATION", "b-green", onMobileVerify), button("AUTO", "b-orange", () => setStatus("Auto login…")));
        const row2 = row(button("LOGIN WITH PASSWORD", "b-blue", onLoginWithPassword), button("AUTO", "b-orange", onLoginWithPassword));
        const row3 = row(button("LOGIN WITH OTP", "b-purple", onLoginWithOtp), button("AUTO", "b-orange", onLoginWithOtp));
        const copyBtn = button("COPY ACCESS TOKEN", "b-purple", () => { navigator.clipboard.writeText(bearerToken || "");
            toast("Access token copied"); });
        const rowStop = row(button("STOP CURRENT", "b-red", stopCurrent), button("STOP ALL", "b-red", stopAll));
        ...
        async function onMobileVerify() {
            ...
            const token = localStorage.getItem(TS_KEY) || (document.querySelector('input[name="captcha_token"]') || {}).value || (document.querySelector('input[name="cf-turnstile-response"]') || {}).value || "";
            if (!token) { setStatus("Solve Turnstile first, then try.", "#b45309");
                focusTurnstileWidget(); return; }
            const ok = nativeMobileVerify(num);
            if (ok) { setStatus("Submitting mobile-verify via native form…", "#0ea5e9");
                toast("Submitting…"); }
        }
        async function onLoginWithPassword() {
            ...
            if (res?.ok) { bearerToken = res.data?.access_token || "";
                kv.write("bearerToken", bearerToken);
                toast("Logged in (password) ✓"); } else toast(res?.msg || "Login failed", false);
        }
        async function onLoginWithOtp() {
            ...
            try {
                ...
                if (res?.ok) { bearerToken = res.data?.access_token || "";
                    kv.write("bearerToken", bearerToken);
                    toast("OTP verified! Token saved ✓"); } else toast(res?.msg || "OTP failed", false);
    ...
    function bgdView() {
        ...
        const note = input("Ready for BGD form submission");
        note.disabled = true;
        const tokenInp = tokenInput();
        tokenInp.value = bearerToken;
        tokenInp.oninput = () => { bearerToken = tokenInp.value.trim();
            kv.write("bearerToken", bearerToken); };
        const rowA = row(button("APPLICATION", "b-blue", submitApplication), button("PERSONAL", "b-green", submitPersonal));
        const rowB = row(button("OVERVIEW", "b-purple", submitOverview), button("SEND OTP", "b-orange", () => sendPayOtp(false)));
        const otpInp = input("Enter 6-digit Payment OTP");
        const rowC = row(button("RESEND OTP", "b-blue", () => sendPayOtp(true)), button("AUTO", "b-orange", () => sendPayOtp(true)));
        const rowD = row(button("VERIFY OTP", "b-purple", verifyPayOtp), button("AUTO", "b-orange", verifyPayOtp));
        ...
        async function submitApplication() { const res = await postFormMulti("appSubmit", kv.read("applicationInfo"), false, true);
            res?.ok ? toast(res.msg || "Application submitted ✓") : toast(res?.msg || "Application failed", false); }
        async function submitPersonal() { const res = await postFormMulti("perSubmit", kv.read("personalInfo"), false, true);
            res?.ok ? toast(res.msg || "Personal submitted ✓") : toast(res?.msg || "Personal failed", false); }
        async function submitOverview() { const res = await postFormMulti("overview", {}, false, true);
            res?.ok ? toast(res.msg || "Overview submitted ✓") : toast(res?.msg || "Overview failed", false); }
        async function sendPayOtp(resend = false) { const res = await postFormMulti("payOtpSend", { resend: resend ? 1 : 0 }, false, true);
    ...
    function payView() {
        const box = document.createElement("div");
        box.style.display = "none";
        const note = input("Ready for payment");
        note.disabled = true;
        const date = input("mm/dd/yyyy", "date");
        const rowA = row(button("GET SLOTS", "b-blue", getSlots), button("AUTO", "b-orange", getSlots));
        const rowB = row(button("PAY NOW", "b-purple", payNow), button("AUTO", "b-orange", payNow));
        const stopBtn = button("STOP ALL", "b-red", stopAll);
        box.append(note, date, rowA, rowB, stopBtn);
        async function getSlots() {
            const d = date.value;
            if (!d) { toast("Select date", false); return; }
        ...
        async function payNow() {
            ...
            const payload = { appointment_date: d, appointment_time: pick, selected_payment: { name: "VISA", slug: "visacard", link: "https://securepay.sslcommerz.com/gwprocess/v4/image/gw1/visa.png" } };
    ...
    function openDataModal() {
        ...
        modal.className = "alx-modal";
        const card = document.createElement("div");
        card.className = "alx-modal-card";
        modal.appendChild(card);
        const title = document.createElement("div");
        title.className = "alx-modal-title";
        title.textContent = "Custom Data Input";
        ...
        const inputs = {};
        fields.forEach(([k, lab]) => { const i = document.createElement("input"); i.className = "alx-input"; i.placeholder = lab; i.value = kv.read("data." + k, "");
            inputs[k] = i;
            card.appendChild(i); });
        const save = document.createElement("button");
        save.className = "alx-btn b-green";
        save.textContent = "Save Data";
        save.onclick = () => {
            const data = {};
    ...
    /************** STOP **************/
    function stopCurrent() { const last = Array.from(active.values()).pop(); if (last) { last.abort();
            toast("Stopped current"); } }

    function stopAll() { active.forEach(ac => ac.abort());
        active.clear();
        toast("Stopped all"); }
